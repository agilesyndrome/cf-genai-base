...
export interface CommandRunOptions { inherit?: boolean; failOnError?: boolean; timeout?: number; env?: NodeJS.ProcessEnv }
...
export interface VendoredMigrationResult { packageName: string; version?: string; added: string[]; skipped: string[] }
export interface DataAccessViolation { path: string; line: number; text: string }
export interface DataAccessLintResult { skipped: boolean; violations: DataAccessViolation[] }
export interface ReleaseStatusChecks { github_actions: { state: string }; npm: { state: string } }
export interface DevCommandInput { hasScript: boolean; args?: readonly string[] }
export interface RefreshStatusInput { cwd?: string; env?: NodeJS.ProcessEnv }
interface PackageMigration { name: string; path: string; sql: string }
interface VendorMigrationInput { packageName?: string; version?: string; cwd?: string }
interface GithubRun { status?: string; conclusion?: string; url?: string; databaseId?: number; displayTitle?: string; workflowName?: string; headSha?: string }
interface GithubActionsStatus { state: string; runs: GithubRun[]; error?: string }
interface NpmReleaseStatus { state: string; version: string | null; error?: string }
interface BaseReleaseStatus { state: string; version: string | null; latest: string | null; spec?: string; error?: string; upgrade?: string }

function run(command: string, args: readonly string[], { inherit = true, failOnError = true, timeout, env }: CommandRunOptions = {}): SpawnSyncReturns<string> {
...
function packageMigrations(packageName: string, cwd = process.cwd()): PackageMigration[] {
...
function migrationHash(sql: string): string { return createHash("sha256").update(sql).digest("hex").slice(0, 12); }

export function vendorPackageMigrations({ packageName = basePackageName, version, cwd = process.cwd() }: VendorMigrationInput = {}): VendoredMigrationResult {
  ...
  for (const source of sources) {
    ...
    let target: string;
    const packageSuffix = packageName.split("/").at(-1) || "package";
    do { target = `${String(next++).padStart(4, "0")}_${packageSuffix.replaceAll("-", "_")}_${source.name.replace(/^\d+_/, "")}`; } while (used.has(target));
...
export function upgradePackageName(value: unknown): string {
  const key = String(value || "");
  const target = key === "base" || key === "auth" ? upgradePackages[key] : undefined;
  if (!target) throw new Error(`Unknown cf-genai package: ${value || "missing"}. Use base or auth.`);
  return target;
}

function upgradePackage(args: readonly string[] = []): VendoredMigrationResult {
  const target = upgradePackageName(args[0]);
  const requested = args[1] || "latest";
  if (args.length > 2) throw new Error("Upgrade accepts one package and version: cf-genai upgrade <base|auth> <latest|VERSION>.");
...
function packageDependency(packageMetadata: PackageMetadata, name: string): string | null {
  const sections: readonly (keyof Pick<PackageMetadata, "dependencies" | "devDependencies" | "peerDependencies" | "optionalDependencies">)[] = ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"];
  return sections
    .map((section) => packageMetadata[section]?.[name])
    .find((value) => value !== undefined) || null;
}

export function baseDependencyVersion(packageMetadata: PackageMetadata = {}): string | null {
...
function isCfGenaiProject(cwd: string): boolean {
  if (!existsSync(join(cwd, packagePath))) return false;
  const pkg = parsePackageMetadata(readFileSync(join(cwd, packagePath), "utf8"));
  const name = pkg.name || "";
...
export function dataAccessLint({ cwd = process.cwd() }: { cwd?: string } = {}): DataAccessLintResult {
...
function version(): string {
  ...
  if (!value) throw new Error("package.json does not contain a version.");
  return value;
}
...
function assertMainIsSynchronized(): void {
...
function assertFlag(args: readonly string[], flag: string, message: string): void {
  if (!args.includes(flag)) throw new Error(`${message} Re-run with ${flag}.`);
}

function npmVersion(name: string, currentVersion: string): string {
...
function npmVersionState(name: string, requestedVersion: string): { exists: boolean; version: string | null } {
  ...
  if (result.status === 0) {
    let published: unknown = result.stdout.trim();
    try { published = JSON.parse(result.stdout.trim()); } catch { /* npm may return an unquoted version */ }
    const publishedVersion = typeof published === "string" ? published : null;
    return { exists: publishedVersion === requestedVersion, version: publishedVersion };
  }
...
export function requestedReleaseAction(currentVersion: string, requestedVersion: string): { version: string; bump: boolean } {
...
export function isNpmAuthenticationFailure(output = ""): boolean {
...
function ensureNpmLogin(name: string): void {
...
function addNpmTrust(name: string): SpawnSyncReturns<string> {
...
export function releaseWaitMinutes(args: readonly string[] = []): number {
...
export function releaseStatusShouldContinue(checks: ReleaseStatusChecks, now: number, deadline: number): boolean {
...
export function releaseStatusDot(state: string, ready = false): "🟢" | "🔴" | "🟡" {
...
interface ReleaseTagStatus { present: boolean; local_sha: string | null; remote_sha: string | null; matches: boolean }
interface GitReleaseStatus { clean: boolean; branch: string | null; pushed: boolean; head_sha: string | null; remote_sha: string | null }

function releaseTagState(tag: string): ReleaseTagStatus {
...
function gitReleaseState(): GitReleaseStatus {
...
function githubActionsState(repository: string, commit: string, timeout: number): GithubActionsStatus {
  ...
  let parsed: unknown;
  try { parsed = JSON.parse(result.stdout || "[]"); } catch { return { state: "unavailable", runs: [], error: "GitHub Actions returned invalid JSON." }; }
  if (!Array.isArray(parsed)) return { state: "unavailable", runs: [], error: "GitHub Actions returned invalid JSON." };
  const runs = parsed.filter(isGithubRun);
...
function npmReleaseState(name: string, currentVersion: string, timeout: number): NpmReleaseStatus {
  const result = run("npm", ["view", `${name}@${currentVersion}`, "version", "--json", "--registry", npmRegistry], { inherit: false, failOnError: false, timeout });
  if (result.status === 0) {
    let published: unknown = result.stdout.trim();
    try { published = JSON.parse(result.stdout.trim()); } catch { /* npm may return an unquoted version */ }
    const publishedVersion = typeof published === "string" ? published : null;
    return { state: publishedVersion === currentVersion ? "published" : "not_published", version: publishedVersion };
  }
...
function baseReleaseState(packageMetadata: PackageMetadata, timeout: number): BaseReleaseStatus {
  ...
  let latest: unknown = result.stdout.trim();
  try { latest = JSON.parse(result.stdout.trim()); } catch { /* npm may return an unquoted version */ }
  if (typeof latest !== "string" || !latest) return { state: "unavailable", version: configured, latest: null, spec };
...
export function devCommand({ hasScript, args = [] }: DevCommandInput): string[] {
  const forwarded = args[0] === "--" ? args.slice(1) : args;
  const command = hasScript ? ["npm", "run", "dev", ...forwarded] : ["npx", "wrangler", "dev", ...forwarded];
  return ["op", "run", "--env-file=.env.dev", "--", ...command];
}

export function lastProdRefresh({ cwd = process.cwd(), env = process.env }: RefreshStatusInput = {}): string | null {
...
function printDevDataStatus(options: RefreshStatusInput = {}): void {
...
function applyDevMigrations(env: NodeJS.ProcessEnv = process.env): void {
  ...
  migrateD1({
    ...
    options: {
      ...
      database: env.CF_GENAI_DATABASE || "DB",
      productionDatabase: env.CF_GENAI_DATABASE || "DB",
...
function publishFirst(args: readonly string[]): SpawnSyncReturns<string> {
...
function addTrust(args: readonly string[]): SpawnSyncReturns<string> {
...
function release(args: readonly string[]): SpawnSyncReturns<string> | undefined {
  ...
  if (!type || !["patch", "minor", "major"].includes(type)) throw new Error("--type must be patch, minor, or major.");
...
interface ReleaseChecks {
  git: GitReleaseStatus;
  tag: ReleaseTagStatus;
  base: BaseReleaseStatus;
  github_actions: GithubActionsStatus;
  npm: NpmReleaseStatus;
}
...
export type ProjectCommandResult = SpawnSyncReturns<string> | DataAccessLintResult | VendoredMigrationResult | ReleaseStatusResult | Promise<ReleaseStatusResult> | undefined | null;

async function releaseStatus(args: readonly string[] = []): Promise<ReleaseStatusResult> {
  const name = packageName();
  const currentVersion = version();
  ...
  const checks: ReleaseChecks = { git: gitReleaseState(), tag: releaseTagState(tag), base: baseReleaseState(packageJson(), releaseStatusCommandTimeout), github_actions: { state: "pending", runs: [] }, npm: { state: "not_published", version: null } };
...
function parsePackageMetadata(text: string): PackageMetadata {
  ...
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new Error("package.json must contain a JSON object.");
  const metadata: PackageMetadata = {};
  const name = Reflect.get(value, "name");
  const packageVersion = Reflect.get(value, "version");
  if (typeof name === "string") metadata.name = name;
  if (typeof packageVersion === "string") metadata.version = packageVersion;
  for (const field of ["scripts", "dependencies", "devDependencies", "peerDependencies", "optionalDependencies"] as const) {
    const entries = stringRecord(Reflect.get(value, field));
    if (entries) metadata[field] = entries;
  }
  return metadata;
}
...
function stringRecord(value: unknown): Record<string, string> | null {
  ...
  if (!entries.every((entry) => typeof entry[1] === "string")) return null;
  return Object.fromEntries(entries);
}

function isGithubRun(value: unknown): value is GithubRun {
  return value !== null && typeof value === "object" && !Array.isArray(value)
    && (Reflect.get(value, "status") === undefined || typeof Reflect.get(value, "status") === "string")
    && (Reflect.get(value, "conclusion") === undefined || typeof Reflect.get(value, "conclusion") === "string")
    && (Reflect.get(value, "url") === undefined || typeof Reflect.get(value, "url") === "string");
}
