...
export function createWorker({ fetch, scheduled, app = { name: "worker" }, auth, authorize, scopes = [], subscriptionManifest = [], scopeRoutes = [], apiRoutes = [], middleware = [], features = [], dataResources = [], repositories = [], publicTenantId = null, health, boot, security = true, adminPage, siteAdminPage }) {
  ...
  const application = defineApp(app); const provider = auth || features.find((feature) => typeof feature?.getUser === "function"); const eventHandler = createEventHandler(features);
  ...
  return {
    async fetch(request, env, ctx) {
      try {
        ...
        const response = await dispatch(0); return withRequestId(security ? secureResponse(response) : response, state.requestId);
      } catch (error) {
