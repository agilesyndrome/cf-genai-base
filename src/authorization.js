...
export async function ensureUser(env, user, { who = "system:read" } = {}) {
  ...
  if (existing) {
    await db.prepare(`UPDATE ${AUTH_USER_TABLE} SET email=?,display_name=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(email, String(user.name || email || subject), existing.id).run();
    await ensureDefaultTenantMembership(db, existing.id);
    return { ...existing, email, display_name: String(user.name || email || subject), is_admin: Boolean(existing.is_admin) };
  }
